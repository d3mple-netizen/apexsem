...
export const trafficDict = defineDict(
  ...
  },
  {
    ...
    subtitle:
      'Примерная модель: как горячие запросы делятся между вами и главными конкурентами, какие запросы вы теряете и что с каждым делать.',
    ...
    plannedBody:
      'На сайте и в рекламных кабинетах ничего не меняется. Внедрите правки ниже и перезапусти анализ, чтобы увидеть результат.',
    ...
    queriesTitle: 'Запросы, которые вы теряете',
    ...
    ctaBody: (price: number) =>
      `Pro стоит $${price} в месяц: анализы без лимита, приоритетный чат со стратегом и созвон, на котором вместе настроим первые кампании. В ваши кабинеты без вас не заходим.`,
