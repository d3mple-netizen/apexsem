...
export const croDict = defineDict(
  ...
  },
  {
    ...
    subtitle: 'Платный клик окупается, только если страница конвертирует. Смотрим, совпадает ли она с объявлениями, понятна ли и где люди уходят.',

    headlineScore: 'Понятность заголовка',
    headlineHint: 'Проверяем, что H1 один и читается с одного взгляда.',
    matchScore: 'Совпадение с объявлением',
    matchHint: 'Насколько страница подтверждает обещания рекламы. От этого зависит Quality Score в Google Ads, а значит и CPC.',
    frictionScore: 'Барьеры регистрации',
    frictionHint: 'Чем ниже, тем лучше. Считаем поля формы и серьезные барьеры на пути к регистрации.',

    heroTitle: 'Новый первый экран',
    copyText: 'Копировать',
    copied: 'Скопировано',
    ...
    issue: 'Проблема:',
    fix: 'Решение:',
    noFindings: 'На главной серьезных барьеров нет.',

    formTitle: 'Форма в одно поле',
    formBody: (domain: string) => `Набросок регистрации в одно поле для ${domain}. Данные никуда не отправляются.`,
    workEmail: 'Рабочий email',
    formNote: 'Одно поле, без пароля и карты. Остальное спросите после регистрации.',
    previewDone: 'Вот и вся регистрация',
    previewDoneHint: 'Это превью, данные никуда не отправлены.'
  }
);
