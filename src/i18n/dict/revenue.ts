...
export const revenue = defineDict(
  ...
  },
  {
    ...
    subtitle: 'Модель "что если": задай средний чек и конверсии воронки. Спрос в поиске для',
    ...
    captureAssumption: (pct: number, demand: string) =>
      `Считаем, что ты забираешь ${pct}% смоделированного спроса в нише (${demand} запросов/мес).`,
    ...
    summary: (acv: string, demand: string, visits: string, arr: string, ratio: string) => [
      ...
      ' при LTV:CAC около ',
      ratio,
      '. Это прикидка по твоим вводным, а не обещание.'
    ],
    csv: {
      scenario: 'Сценарий',
      acv: 'ACV ($)',
